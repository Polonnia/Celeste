...
#define VSPRINTF vsprintf_s    // 为Visual Studio添加安全版本的vsprintf
#elif __linux__
...
void _log(const char* prefix, const char* msg, TextColor textColor, Args... args)
{
  ...
  VSPRINTF(textBuffer, sizeof(textBuffer), formatBuffer, args...);
#else
  sprintf(formatBuffer, "%s %s %s \033[0m", TextColorTable[textColor], prefix, msg);
  vsprintf(textBuffer, formatBuffer, args...);
#endif
...
{                                            \
  ...
  {                                         \
    ...
    sprintf_s(buffer + 7, sizeof(buffer) - 7, msg, ##__VA_ARGS__);  \
    puts(buffer);                           \
    DEBUG_BREAK();                          \
  }                                         \
}
